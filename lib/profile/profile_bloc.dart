...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  ProfileBloc({required this.dataRepo, required this.storageRepo, required User user})
    ...
    if (user.avatarKey != null) {
      storageRepo.getUrl4PublicFile(user.avatarKey!).then((url) => add(ProvideImagePath(avatarPath: url)));
    }
  ...
  void _openImagePicker(OpenImagePicker event, Emitter<ProfileState> emit) async {
    ...
    final imageKey = await storageRepo.uploadPublicFile(category: StorageCategory.avatar, file: File(pickedImage.path));
