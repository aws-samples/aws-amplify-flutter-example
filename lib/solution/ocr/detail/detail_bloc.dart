...
class OCRDetailBloc extends Bloc<OCRDetailEvent, OCRDetailState> {
  ...
  void _refresh(OCRDetailRefreshEvent event, Emitter<OCRDetailState> emit) async {
  ...
  Future<void> _loadData(String id, Emitter<OCRDetailState> emit) async {
    try {
      ...
      emit(state.copyWith(loadingState: LoadingState.success, record: await _parse(record!)));
